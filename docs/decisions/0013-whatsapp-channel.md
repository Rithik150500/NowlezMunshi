...
- Inbound routing parses a **command set** (`case`/`orders`/`file`/`cause-list`/`help`, a bare CNR;
  anything else → Munshi) via `parseWhatsAppCommand` + the server's `handleWhatsAppText`, and
  `WhatsAppClient.sendDocument` delivers a stored file as **media** (Meta upload-then-send).
  **Rendering** order/cause-list PDFs (for media) and webhook **signature verification** remain open.
